...
public class MySQLProvider implements IStorageProvider {
    ...
    public static final String CREATE_STATS_TABLE = String.format("CREATE TABLE IF NOT EXISTS %s (%s VARCHAR(10), %s VARCHAR(48), %s VARCHAR(64), %s DOUBLE, PRIMARY KEY (%s, %s, %s));",
            TABLE_NAME, COLUMN_TARGET_TYPE, COLUMN_TARGET_ID, COLUMN_STAT_ID, COLUMN_VALUE, COLUMN_TARGET_TYPE, COLUMN_TARGET_ID, COLUMN_STAT_ID);
